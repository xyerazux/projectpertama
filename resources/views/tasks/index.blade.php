    ...
    <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-6">
        <form action="{{ route('tasks.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
            <div class="relative w-full sm:w-48">
                <select name="priority" onchange="this.form.submit()" class="appearance-none w-full bg-gray-50 border-none text-gray-600 text-xs font-bold rounded-xl px-4 py-3 sm:py-2.5 pr-10 focus:ring-2 focus:ring-indigo-100 cursor-pointer">
            ...
            <div class="relative w-full sm:w-48">
                <select name="category_id" onchange="this.form.submit()" class="appearance-none w-full bg-gray-50 border-none text-gray-600 text-xs font-bold rounded-xl px-4 py-3 sm:py-2.5 pr-10 focus:ring-2 focus:ring-indigo-100 cursor-pointer">
            ...
            <table class="w-full min-w-[800px]">
                <thead class="bg-gray-50/50 border-b border-gray-100">
                    <tr class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
                        ...
                        <th class="px-6 py-5 text-center">Priority Status</th>
                    ...
                    <tr class="hover:bg-indigo-50/10 transition-all group">
                        ...
                        <td class="px-6 py-6 text-center">
                            <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest 
                                {{ $task->priority == 'high' ? 'bg-red-50 text-red-500' : ($task->priority == 'medium' ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600') }}">
                                {{ $task->priority }}
                            </span>
                        </td>
                        <td class="px-6 py-6 text-center">
                        ...
                        <td class="px-8 py-6 text-right">
                            <div class="flex justify-end items-center gap-1">
                                <a href="{{ route('tasks.edit', $task->id) }}" class="p-2 text-blue-400 hover:bg-blue-50 rounded-xl transition-all"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></a>
                                
                                <form action="{{ route('tasks.done', $task->id) }}" method="POST">
                                    @csrf 
                                    @method('PATCH')
                                    <button type="submit" class="p-2 text-emerald-400 hover:bg-emerald-50 rounded-xl transition-all"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></button>
                                </form>

                                {{-- Form Delete dengan Peringatan --}}
                                <form action="{{ route('tasks.destroy', $task->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to move this task to trash?')">
                                    @csrf 
                                    @method('DELETE')
                                    <button type="submit" class="p-2 text-red-300 hover:bg-red-50 rounded-xl transition-all"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                                </form>
                            </div>
                        </td>
                    </tr>
